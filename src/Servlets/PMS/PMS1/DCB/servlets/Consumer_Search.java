    ...
    public void doGet(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException
    {
       Connection connection=null;
        ...
        try
        {
            ...
            ResultSet res =   statement.executeQuery("Select rd_panchayat_code from com_mst_panchayats "
            		+ "where panch_sno = "+pan);
     
            while(res.next())
            {
            	
            	int lgdCode = res.getInt("rd_panchayat_code");
            	System.out.println("Lgd Code :" +lgdCode);
            }
        ...
        else if(strCommand.equals("Panch"))
        {
            ...
            try
            {
	             result = statement.executeQuery("SELECT COUNT(*) AS rec " +
												...
												"  WHERE district_code::numeric in (" + dis + ")" +
	             ...
	             try
	             {
	                 ...
	                 while(result.next())
	                 {
	          		 	xml+= "<row>";
		     				...
		     				xml+= "<lgd>" + result.getString("rd_panchayat_code") + "</lgd>";
...
//        if(strCommand.equals("RdHabitation")) {
//            System.out.println("\n*************\nblock\n**************\n");
//            xml = "<response><command>RdHabitation</command>";
//
//            try {
//                String sql;
//                if ("%".equals(blk)) {
//                    // All blocks in district, don't filter by block
//                    sql = "SELECT * FROM rd_directory WHERE rd_district_code = (" +
//                          "SELECT rd_district_code FROM com_mst_districts WHERE district_code = " + dis + ")";
//                } else {
//                    // Specific block
//                    sql = "SELECT * FROM rd_directory WHERE rd_district_code = (" +
//                          "SELECT rd_district_code FROM com_mst_districts WHERE district_code = " + dis + ") " +
//                          "AND rd_block_code = (" +
//                          "SELECT rd_block_code FROM com_mst_blocks WHERE block_sno::varchar LIKE '" + blk + "')";
//                }
//                result = statement.executeQuery(sql);
//                xml = xml + "<flag>success</flag>";
//                while(result.next()) {
//                    xml += "<row>";
//                    xml += "<rd_panchayat_code>" + result.getInt("rd_panchayat_code") + "</rd_panchayat_code>";
//                    xml += "<rd_panchayat_name>" + result.getString("rd_panchayat_name") + "</rd_panchayat_name>";
//                    xml += "</row>";
//                }
//                result.close();
//            } catch(Exception e1) {
